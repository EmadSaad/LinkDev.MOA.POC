...
export class AvailableBiddingsService implements WorkspaceService<IBiddingsFiltration, IBidding> {
  ...
  search(filter: IBiddingsFiltration): Observable<ApiGenericResponse<IGridResultBase<IBidding>>> {
    return this.api.Post<ApiGenericResponse<IGridResultBase<IBidding>>>(`${this.portalUrl}/GetAvailableBiddings`, filter).pipe();
  }
}
